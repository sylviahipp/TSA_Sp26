...
create_plots <- function(series, description){
  time_series_plot <- ggplot(energy_data, 
    ...
    labs(x = "Month", 
         ...
         title = glue("Time Series of {description} (1973–2025)")) 
  
  #cowplot::plot_grid(time_series_plot, Acf(series), Pacf(series))
  time_series_plot
}
...
test <- Acf(energy_data$total_renewable_energy_production)
test
```
...
linear_model_renewable <- lm(total_renewable_energy_production~month, energy_data)
summary(linear_model_renewable)

# Hydroelectric Power Consumption
linear_model_hydro <- lm(hydroelectric_power_consumption~month, energy_data)

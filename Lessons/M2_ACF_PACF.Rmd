...
raw_inflow_data <- raw_inflow_data[,1:6] #the space before the comma means you want all rows
...
colnames(raw_inflow_data)=c("Month","Year", "HP1", "HP2","HP3", "HP4")
...
inflow_data <- raw_inflow_data %>% 
  tibble() %>% 
  mutate(my_date = my(paste(Month, Year, "-"))) %>% 
  select(-Month, -Year) %>% 
  relocate(my_date) 
...
ggplot(inflow_data) + 
  geom_line(aes(x = my_date, y = HP1))

#adding two time series to the same plot
inflow_data %>% 
  pivot_longer(cols = c(HP1:HP3), 
               names_to = "reservoir", 
               values_to = "inflow") %>% 
  filter(reservoir != "HP1") %>%   # show just two of the reservoirs on one plot
  ggplot() + 
  geom_line(aes(x = my_date, y = inflow, color = reservoir))
...
```{r}
ts_inflow_data <- ts(inflow_data[,2:5],   # relevant data columns
                     start = c(1931, 1),  # starts Jan 1931
                     frequency = 12)      # Jan repeats every 12 months (monthly)
ts_inflow_data
...
# can also just specify one or two of the ts
...
HP1_acf = acf(ts_inflow_data[,1],   # first inflow time series
              lag.max = 40,         # how many lags to appear in the plot
              plot= TRUE)
# blue dashed lines = confidence interval; Autocorrelation within that gap = effectively zero


HP1_pacf = pacf(ts_inflow_data[, 1], lag.max = 40, plot = TRUE)
...
HP1_acf = forecast::Acf(ts_inflow_data[,1], 
                        lag.max = 40)  # excludes lag = 0
HP1_pacf = forecast::Pacf(ts_inflow_data[,1], lag.max = 40)

HP2_acf = forecast::Acf(ts_inflow_data[,2], lag.max = 40)
HP3_acf = forecast::Acf(ts_inflow_data[,3], lag.max = 40)  # less of a negative autocorrelation 

# HP4
HP4_acf = Acf(ts_inflow_data[,"HP4"], lag.max = 60)
HP4_pacf = Pacf(ts_inflow_data[,"HP4"], lag.max = 60)

HP1_pacf$acf  # have to call 'acf' but this gives value of PACF output

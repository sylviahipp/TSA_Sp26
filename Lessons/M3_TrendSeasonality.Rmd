...
```{r}
# exclude date column at the beginning
ts_inflow_data <- ts(inflow_data[,2:(2+nhydro-1)],
                     frequency=12, 
                     start = c(1931, 1)) 
# add date information with 'start'
...
# for loop for each of reservoirs in the dataset
for(i in 1:nhydro){
...
```{r}
#Create vector t
# t <- seq(from = min(inflow_data$my_date), 
#          to = max(inflow_data$my_date), 
#          by = "month")
# can use date series or just index of obs.
t <- c(1:nobs)

# can do for loop
# for now: choose one HP to work with
iHP <- 4

#Fit a linear trend to TS of iHP
# lm takes data frame, not TS object
linear_trend_model <- lm(inflow_data[,iHP+1] ~ t)
summary(linear_trend_model)  # p-value is significant, there is a significant trend


# use regression coefficient to de-trend
# store coefficients in variables
beta0 <- as.numeric(linear_trend_model$coefficients[1])  # first term is intercept

beta1 <- as.numeric(linear_trend_model$coefficients[2]) # second term is slop
...
# i.e. subtract red line in plot shown above

# creates numeric vector
detrend_inflow_data <- inflow_data[,iHP+1] - (beta0 + beta1*t)

# transform to ts
ts_detrend_inflow <- ts(detrend_inflow_data, 
                        frequency = 12,
                        start = c(1931, 1))
...
```{r}
## ACF and PACF of the detrended series

par(mfrow=c(1,2))
Acf(ts_inflow_data[, iHP], lag.max=40)
Acf(ts_detrend_inflow, lag.max=40)
# see that trend was having an impact on the temporal behavior / dependence of the series 

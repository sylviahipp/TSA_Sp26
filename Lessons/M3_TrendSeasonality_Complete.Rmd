...
> ACF and PACF for these series don't suggest stochastic trend
...
> detrending elimnated some temporal dependence (lower ACF) but did not eliminate it
...
```
> eliminated seasonality in both ACF and PACF
...
inflow_data_yearly <- colMeans(inflow_data_matrix)  # calculate yearly averages
...
# Seasonal Mann-Kendall (test deterministic)
seasonal_mk <- Kendall::SeasonalMannKendall(ts_inflow_data[, iHP])
summary(seasonal_mk)   # p-value < 0.05 --> reject H0 that it's stationary (has deterministic trend)

# more detailed SMK test
summary(trend::smk.test(ts_inflow_data[, iHP]))  # see significant increasing trend for all seasons


## Spearman correlation
ts_matrix <- matrix(ts_inflow_data[,iHP],byrow=FALSE,nrow=12)
ts_yearly <- data.frame(my_year, "value" = colMeans(ts_matrix))

ggplot(ts_yearly, aes(x = my_year, y = value)) + 
  geom_line() + 
  geom_smooth(method = "lm", color = "blue")  # see increasing value 

cor.test(ts_yearly$value, my_year, method = "spearman")  # rho = 0.758 (positive correlation)


## ADF
# run ADF to make sure the trend we're seeing is not stochastic 
adf.test(ts_inflow_data[,iHP], alternative = "stationary")  # Reject H0: stationary over unit root

# ADF on yearly series accepts null hypothesis (that unit root is present)
# in the end: want to accept test results for series you're building model on (monthly data)

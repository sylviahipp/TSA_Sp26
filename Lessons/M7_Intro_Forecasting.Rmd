...
mean_seas <- meanf(ts_electricity_price[,"All.sectors"], h=36)
plot(mean_seas)     # super bad, just plots the mean as straight line
...
naive_seas <- snaive(ts_electricity_price[,"All.sectors"], h=36)
plot(naive_seas)            # what happened the previous year will repeat
...
sma_deseas <- sma(deseasonal_allsectors_price, h=36, silence=FALSE)  # didn't specify order, iterating to find best order
summary(sma_deseas)

# Model 6: Simple moving average on original data
sma_seas <- sma(ts_electricity_price[,"All.sectors"], h=36)

# Model 7:  Simple exponential smoothing on deseasonal data
ses_deseas <- ses(deseasonal_allsectors_price, h=36)

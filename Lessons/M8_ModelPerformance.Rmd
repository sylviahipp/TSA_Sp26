...
n_for <- 12    # set forecasting window

ts_electricity_price <- ts(
  electricity_price_processed[1:(nobs-n_for),2:(nvar+1)],   # drop last obs. in forecasting window
...
```{r}
# meanf = arithmetic mean model
# h = number of periods for forecasting
MEAN_seas <- meanf(y = ts_electricity_price[,"All.sectors"], h = 12)
summary(MEAN_seas)     # shows forecast for the last year of the data (forecasted period)
checkresiduals(MEAN_seas)  # residuals relative to actual values
...
Seasonal Naive repeats observations from the previous observation of the same season (i.e., month)

```{r}
SNAIVE_seas <- snaive(ts_electricity_price[,"All.sectors"], h=12)
summary(SNAIVE_seas)  # see some seasonal variation
...
### Model 3: SARIMA
Note: SARIMA must fit to the model, but doesn't forecast
need to call forecast separately
```{r}
SARIMA_autofit <- auto.arima(ts_electricity_price[,"All.sectors"])
checkresiduals(SARIMA_autofit)   # ACF shown is the ACF of the residuals 
```
...
```{r}
print(class(MEAN_seas))
print(class(SARIMA_autofit))
```
...
```{r}
# last 12 observations = the testing window
observed <- electricity_price_processed[(nobs-n_for+1):nobs, "All.sectors"]

#Model 1: Arithmetic mean
# for this model type, $mean is the point forecast (has 12 observations)
...
SARIMA_for <- forecast(SARIMA_autofit,h=12)  # returns object of class forecast
class(SARIMA_for)
...
```{r}
# what to show observed about AND the test period 

# need to create time series object with all observations up to 2025
ts_observed <- ts(
  electricity_price_processed[, "All.sectors"], 
  start = c(year(first(electricity_price_processed$Month)), 
            month(first(electricity_price_processed$Month))), 
  frequency = 12)

autoplot(ts_observed) + 
  autolayer(SARIMA_for$mean, series = "SARIMA") +
  autolayer(MEAN_seas$mean, series = "Mean") + 
  autolayer(SNAIVE_seas$mean, series = "Seasonal Naive") + 
  theme_classic() + 
  theme(legend.position = "bottom") + 
  labs(y = "Forecasted Electricity Price (All Sectors)", 
